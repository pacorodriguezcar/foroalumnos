 ...
 */
class LogoFooter extends Abstract_Component {


	const COMPONENT_ID = 'logo-footer';
	const LOGO         = 'logo';
	...
	public function init() {
		$this->set_property( 'label', __( 'Logo', 'solace' ) );
		
		$this->set_property( 'id', $this->get_class_const( 'COMPONENT_ID' ) );
		$this->set_property( 'component_slug', 'hfg-logo-footer' );
		$this->set_property( 'width', 3 );
		$this->set_property( 'icon', 'admin-appearance' );
		$this->set_property( 'section', 'logo_footer' );
	...
	public function load_scripts() {
		if ( $this->is_component_active() || is_customize_preview() ) {
			wp_add_inline_script( 'solace-script', $this->toggle_script() );
		}
	}
...
JS;
	...
	public function add_settings() {
		...
		SettingsManager::get_instance()->add(
			[
				...
				'type'              => '\Solace\Customizer\Controls\React\Logo_Palette',
				'options'           => [
					'priority'    => 0,
					'input_attrs' => [
						'builderListen' => 'hfg_header_layout' . ( solace_is_new_builder() ? '_v2' : '' ),
						'compChange'    => PaletteSwitch::COMPONENT_ID,
						'sameLabel'     => __( 'Use one logo for both modes', 'solace' ),
		...
		// 		'default'            => 2,
		// 		'label'              => __( 'Show Site Tagline', 'solace' ),
		...
		SettingsManager::get_instance()->add(
			[
				...
				'default'           => '{ "mobile": "120", "tablet": "120", "desktop": "200" }',
				'label'             => __( 'Logo Width', 'solace' ),
		...
		SettingsManager::get_instance()->add(
			[
				...
				'conditional_header' => $this->get_builder_id() === 'footer',
			]
		);
	}
	...
	public function render_component() {
		Main::get_instance()->load( 'components/component-logo-footer' );
	}
		...
		$css_array[] = [
			...
			Dynamic_Selector::KEY_RULES    => [
				'--maxwidth' => [
					...
					Dynamic_Selector::META_DEFAULT       => '{ "mobile": "120", "tablet": "120", "desktop": "200" }',
				],
				'--color'    => [
	...
	private function add_legacy_style( $css_array ) {
		...
		$css_array[] = [
			...
			Dynamic_Selector::KEY_RULES    => [
				\Solace\Core\Settings\Config::CSS_PROP_MAX_WIDTH => [
					Dynamic_Selector::META_IS_RESPONSIVE => true,
					Dynamic_Selector::META_KEY           => $this->get_id() . '_' . self::MAX_WIDTH,
					Dynamic_Selector::META_DEFAULT       => '{ "mobile": "120", "tablet": "120", "desktop": "200" }',
				],
		...
		$css_array[] = [
			...
			Dynamic_Selector::KEY_RULES    => [
				\Solace\Core\Settings\Config::CSS_PROP_COLOR => $this->get_id() . '_' . self::COLOR_ID,
			],
		];
