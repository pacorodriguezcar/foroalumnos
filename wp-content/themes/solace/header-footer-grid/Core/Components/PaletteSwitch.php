...
class PaletteSwitch extends Abstract_Component {
	...
	public function init() {
		$this->set_property( 'label', __( 'Palette Switch', 'solace' ) );
	...
	private function get_light_dark_palettes() {
		...
		$customizer       = Mods::get( 'solace_global_colors', solace_get_global_colors_default( true ) );
	...
	public function toggle_elementor_css( $css ) {
		...
		return $css . '
		[data-solace-theme="light"], html.solace-light-theme {
			' . $light_css . '
		}
		[data-solace-theme="dark"], html.solace-dark-theme ~ * {
			' . $dark_css . '
		}
		';
	}
	...
	public function add_settings() {
		...
		SettingsManager::get_instance()->add(
			[
				...
				'label'       => __( 'Light Palette', 'solace' ),
				/* translators: %s: Link to Edit global color customizer. */
				'description' => sprintf( __( 'The base palette is used for light mode. %1$sEdit color palettes%2$s.', 'solace' ), '<br/><a onclick="wp.customize.control(\'solace_global_colors\').focus()" href="javascript:void(0);">', '</a>' ),
				'type'        => 'hidden',
				'default'     => '',
			]
		);
