...
use Solace\Core\Styles\Css_Prop;
...
abstract class Abstract_Builder implements Builder {
	...
	const TEXT_COLOR_CLEAR   = 'new_text_color_clear';
	...
	/**
	 * Main Row Padding settings values.
	 *
	 * @var array
	 */
	protected $main_padding_value = array(
		'mobile'       => array(
			'top'    => 48,
			'right'  => 0,
			'bottom' => 24,
	...
	/**
	 * Bottom Row Padding settings values.
	 *
	 * @var array
	 */
	protected $bottom_padding_value = array(
		'mobile'       => array(
			'top'    => 16,
			'right'  => 0,
			'bottom' => 16,
	...
	public function __construct() {
		...
		$this->set_property( 'layout_control_id', $this->control_id . ( solace_is_new_builder() ? '_v2' : '' ) );
		...
		add_filter( 'hfg_page_footer_row_classes', [ $this, 'add_footer_row_utility_classes' ], 10, 2 );
	}
	...
	public function define_builder_settings() {
		...
		SettingsManager::get_instance()->add(
			[
				...
				'default'            => '',
				'label'              => '',
				'type'               => $is_new_builder ? '\Solace\Customizer\Controls\React\Builder' : 'text',
				'options'            => [
	...
	public function define_row_settings( $row_id ) {
		...
		if ( $row_id !== 'sidebar' ) {
			SettingsManager::get_instance()->add(
				[
					...
					'label'              => __( 'Layout', 'solace' ),
					'type'               => 'select',
					'section'            => $row_setting_id,
					'options'            => [
						'choices' => [
							'layout-custom'      => __( 'Custom', 'solace' ),
							'layout-boxed'      => __( 'Boxed', 'solace' ),
							'layout-fullwidth'      => __( 'Full Width', 'solace' ),
						],
					],
					'conditional_header' => $this->get_id() === 'header',
					...
					'default'            => 'layout-custom',
				]
			);

			SettingsManager::get_instance()->add(
				[
					'id'                    => self::LAYOUT_WIDTH,
					'group'                 => $row_setting_id,
					'tab'                   => SettingsManager::TAB_STYLE,
					'section'               => $row_setting_id,
					'label'                 => __( 'Custom', 'solace' ),
					...
					'default'               => '{"desktop": "1280", "tablet": "768", "mobile": "580"}',
					'conditional_header'    => $this->get_id() === 'header',
				]
			);			
		...
		if ( $this->columns_layout && solace_is_new_builder() ) {
		...
		SettingsManager::get_instance()->add(
			[
				...
				'section'               => $row_setting_id,
				'label'                 => __( 'Row Background', 'solace' ),
		...
		SettingsManager::get_instance()->add(
			[
				...
				'group'                 => $row_setting_id,
				'tab'                   => SettingsManager::TAB_GENERAL,
				'label'                 => __( 'Link Hover Color', 'solace' ),
				'section'               => $row_setting_id,
				'type'                  => 'solace_color_control',
				'transport'             => 'postMessage',
				'live_refresh_selector' => $row_class,
				...
				'sanitize_callback'     => 'wp_filter_nohtml_kses',
				'default'               => $default_colors['link_hover'],
			]
		);
		...
		switch ( $row_id ) {
			case 'main':
				$row_padding_value = $this->main_padding_value;
				break;
			case 'bottom':
				$row_padding_value = $this->bottom_padding_value;
				break;				
			default:
				$row_padding_value = $this->default_padding_value;
				break;
		}

		$default_padding = $this->get_default_row_padding( $row_id );
		SettingsManager::get_instance()->add(
			[
				'id'                    => self::PADDING,
				'group'                 => $row_setting_id,
				...
				'live_refresh_css_prop' => array(
					'cssVar' => [
						'vars'       => '--padding',
						'responsive' => true,
						'selector'             => '.' . $this->get_id() . '-' . $row_id . ' .my-row-inner',
					],
					'prop'   => 'padding',
				),
				'transport'             => 'postMessage',
				...
				'default'               => $default_padding['padding'],
			]
		);		
	...
	public function has_background_setting( $row_id ) {
		...
		if ( get_theme_mod( 'solace_pro_global_header_settings_advanced_style', true ) === true ) {
			return true;
		}
		...
		if ( empty( $this->active_components ) ) {
			...
			foreach ( $this->get_layout_data() as $devices ) {
				foreach ( $devices as $row_index => $row ) {
					if ( solace_is_new_builder() && $row_index !== 'sidebar' ) {
						foreach ( $row as $slot ) {
	...
	private function add_row_style( $row_index, $css_array = array() ) {
		...
		if ( $row_index !== 'sidebar' ) {
			...
			$rules['--padding'] = [
				Dynamic_Selector::META_KEY           => $this->control_id . '_' . $row_index . '_' . self::PADDING,
				Dynamic_Selector::META_IS_RESPONSIVE => true,
				Dynamic_Selector::META_DEFAULT       => $default_padding_value,
				Dynamic_Selector::META_SUFFIX        => 'responsive_unit',
				'directional-prop'                   => Config::CSS_PROP_PADDING,
			];	

			$rules['--margin'] = [
				Dynamic_Selector::META_KEY           => $this->control_id . '_' . $row_index . '_' . self::MARGIN,
				Dynamic_Selector::META_IS_RESPONSIVE => true,
				Dynamic_Selector::META_DEFAULT       => SettingsManager::get_instance()->get_default( $this->get_id() . '_' . self::MARGIN ),
				Dynamic_Selector::META_SUFFIX        => 'responsive_unit',
		...
		$rules['--color-clear'] = [
			Dynamic_Selector::META_KEY     => $this->control_id . '_' . $row_index . '_' . self::TEXT_COLOR_CLEAR,
			Dynamic_Selector::META_DEFAULT => 'var(--sol-color-base-font)',
		];

		$rules['--link-color'] = [
			Dynamic_Selector::META_KEY     => $this->control_id . '_' . $row_index . '_' . self::LINK_COLOR,
			Dynamic_Selector::META_DEFAULT => 'var(--sol-color-heading)',
		...
		if ( 'hfg_footer_layout' === $this->control_id  ) {
			if ( 'top' === $row_index || 'main' === $row_index ) {
				$rules['--link-color'] = [
					Dynamic_Selector::META_KEY     => $this->control_id . '_' . $row_index . '_' . self::LINK_COLOR,
					Dynamic_Selector::META_DEFAULT => '#9ca3af',
					Dynamic_Selector::META_FILTER        => function ( $css_prop, $value, $meta, $device ) {
						if ($value == 'inherit') {
							$value = 'var(--sol-color-link-button-initial)';
						}
		...
		$rules['--link-hover-color'] = [
			Dynamic_Selector::META_KEY     => $this->control_id . '_' . $row_index . '_' . self::LINK_HOVER_COLOR,
			Dynamic_Selector::META_DEFAULT => 'var(--sol-color-button-initial)',
			Dynamic_Selector::META_FILTER        => function ( $css_prop, $value, $meta, $device ) {
				if ($value == 'inherit') {
					$value = 'var(--sol-color-link-button-hover)';
				}
				return sprintf( '%s:%s;', $css_prop, $value );
			},
		];
		...
		if ( get_theme_mod( 'solace_pro_global_header_settings_advanced_style', true ) === true || $this->get_id() !== 'header' || $row_index === 'sidebar' ) {
			// If there is no default, use site background.
			$default_color = isset( $default_colors['background'] ) ? $default_colors['background'] : 'var(--sol-color-background)';
			...
			if ( $background['type'] === 'color' && ! empty( $background['colorValueElementor'] ) && $background['toggleElementor'] ) {
				$rules = array_merge(
					$rules,
					[
						'--bgcolorelementor' => [
							Dynamic_Selector::META_KEY     => $this->control_id . '_' . $row_index . '_background.colorValueElementor',
							Dynamic_Selector::META_DEFAULT => $default_color,
						],
					]
				);
	...
	public function new_render_components( $device ) {
		...
		foreach ( $render_buffer as $slot => $slot_data ) {
			...
			do_action( 'solace_after_slot_component', $this->get_id(), $row_index, $slot );
	...
	protected function get_default_row_colors( $row_id ) {
		$bg_color_map = [
			'background' => [
				'dark-mode'  => 'var(--sol-color-background)',
				'light-mode' => 'var(--sol-color-background)',
			],
			'text'       => [
				'dark-mode'  => 'var(--sol-color-heading)',
				'light-mode' => 'var(--sol-color-heading)',
			],
			'link'       => [
				'dark-mode'  => 'var(--sol-color-heading)',
				'light-mode' => 'var(--sol-color-heading)',
			...
			'link-hover'       => [
				'dark-mode'  => 'var(--sol-color-button-initial)',
				'light-mode' => 'var(--sol-color-button-initial)',
			],
		];

		$is_footer_top = $this->get_id() === 'footer' && $row_id === 'top';

		// On the new skin, the bottom footer row should be dark by default.
		if ( solace_is_new_skin() && $is_footer_top ) {
			$bg_color_map = [
				'background' => [
					'dark-mode'  => 'var(--sol-color-background)',
					'light-mode' => 'var(--sol-color-background)',
				...
				'text'       => [
					'dark-mode'  => 'var(--sol-color-base-font)',
					'light-mode' => 'var(--sol-color-base-font)',
				],
				'link'       => [
					'dark-mode'  => '#9ca3af',
					'light-mode' => '#9ca3af',
		...
		$is_footer_bottom = $this->get_id() === 'footer' && $row_id === 'main' || $row_id === 'bottom';

		// On the new skin, the bottom footer row should be dark by default.
		if ( solace_is_new_skin() && $is_footer_bottom ) {
			$bg_color_map = [
				'background' => [
					'dark-mode'  => 'var(--sol-color-heading)',
					'light-mode' => 'var(--sol-color-heading)',
				...
				'text'       => [
					'dark-mode'  => 'var(--sol-color-page-title-text)',
					'light-mode' => 'var(--sol-color-page-title-text)',
				],
				'link'       => [
					'dark-mode'  => 'var(--sol-color-page-title-text)',
					'light-mode' => 'var(--sol-color-page-title-text)',
		...
		$link           = $bg_color_map['link']['light-mode'];
		$link_hover     = $bg_color_map['link-hover']['light-mode'];
		...
		if ( ! empty( $old_skin ) ) {
			...
			$link       = $bg_color_map['link'][ $old_skin ];
			$link_hover = $bg_color_map['link-hover'][ $old_skin ];
		}
	...
	/**
	 * Get the default row padding based on the old settings.
	 *
	 * @param string $row_id the row id.
	 *
	 * @return array
	 */
	protected function get_default_row_padding( $row_id ) {
		$padding_map = [
			'padding' => [
				'mobile'       => array(
					'top'    => 24,
	 ...
	 * @return array
	 */
	protected function get_default_row_number_of_columns( $row_id ) {
		$value = [
			'columns_number' => '3',
		];

		$is_footer_top = $this->get_id() === 'footer' && $row_id === 'top';
		$is_footer_main = $this->get_id() === 'footer' && $row_id === 'main';
		$is_footer_bottom = $this->get_id() === 'footer' && $row_id === 'bottom';

		// On the new skin, the top footer row should be dark by default.
		if ( solace_is_new_skin() && $is_footer_top ) {
			$value = [
				'columns_number' => '5',
			];
		...
		// On the new skin, the main footer row should be dark by default.
		if ( solace_is_new_skin() && $is_footer_main ) {
			$value = [
				'columns_number' => '1',
			];
		}
		...
		$row_setting_id = $this->control_id . '_' . $row_id;
		$columns_number        = $value['columns_number'];
	...
	private function add_sidebar_controls( $row_setting_id ) {
		$align_choices = [
			'left'   => [
				'tooltip' => __( 'Left', 'solace' ),
				'icon'    => 'editor-alignleft',
			],
			'center' => [
				'tooltip' => __( 'Center', 'solace' ),
				'icon'    => 'editor-aligncenter',
			],
			'right'  => [
				'tooltip' => __( 'Right', 'solace' ),
		...
		SettingsManager::get_instance()->add(
			[
				...
				'tab'                   => SettingsManager::TAB_LAYOUT,
				'transport'             => 'postMessage',
				'label'                 => __( 'Alignment', 'solace' ),
				'type'                  => '\Solace\Customizer\Controls\React\Responsive_Radio_Buttons',
				'section'               => $row_setting_id,
				'options'               => [
		...
		SettingsManager::get_instance()->add(
			[
				...
				'options'            => [
					'choices' => [
						'slide_left'  => __( 'Slide from Left', 'solace' ),
						'slide_right' => __( 'Slide from Right', 'solace' ),
						'pull_left'   => __( 'Pull from Left', 'solace' ),
						'pull_right'  => __( 'Pull from Right', 'solace' ),
						'full_canvas' => __( 'Full Canvas', 'solace' ),
						'dropdown'    => __( 'Slide Down', 'solace' ),
					],
				],
				'conditional_header' => true,
				'transport'          => 'refresh',
	...
	private function add_columns_layout_controls( $row_setting_id, $row_id ) {
		...
		$default_row_number_of_columns = $this->get_default_row_number_of_columns( $row_id );
		SettingsManager::get_instance()->add(
			[
				'id'                => self::COLUMNS_NUMBER,
				'group'             => $row_setting_id,
				'tab'               => SettingsManager::TAB_LAYOUT,
				'label'             => __( 'Number of Columns', 'solace' ),
				...
				'default'           => $default_row_number_of_columns['columns_number'],
			]
		);
		...
		SettingsManager::get_instance()->add(
			[
				...
				'group'             => $row_setting_id,
				'tab'               => SettingsManager::TAB_LAYOUT,
				'label'             => __( 'Columns Layout', 'solace' ),
				'type'              => '\Solace\Customizer\Controls\React\Builder_Columns',
				'section'           => $row_setting_id,
				'options'           => [
		...
		$align_choices = [
			'flex-start' => [
				'tooltip' => __( 'Top', 'solace' ),
				'icon'    => 'arrow-up',
			],
			'center'     => [
				'tooltip' => __( 'Middle', 'solace' ),
				'icon'    => 'sort',
			],
			'flex-end'   => [
				'tooltip' => __( 'Bottom', 'solace' ),
				'icon'    => 'arrow-down',
			],
		];
		...
		SettingsManager::get_instance()->add(
			[
				...
				'default'               => 'flex-start',
				'section'               => $row_setting_id,
				'label'                 => __( 'Vertical Alignment', 'solace' ),
				'type'                  => '\Solace\Customizer\Controls\React\Radio_Buttons',
				'live_refresh_selector' => true,
				'live_refresh_css_prop' => [
					'cssVar' => [
