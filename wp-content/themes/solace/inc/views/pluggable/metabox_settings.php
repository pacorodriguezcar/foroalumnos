 ...
 *
 * @package Solace\Views\Pluggable
 */

namespace Solace\Views\Pluggable;
 ...
 */
class Metabox_Settings {
	const CONTENT_WIDTH          = 'solace_meta_content_width';
	const ENABLE_CONTENT_WIDTH   = 'solace_meta_enable_content_width';
	const CONTAINER              = 'solace_meta_container';
	const SIDEBAR                = 'solace_meta_sidebar';
	const TITLE_ALIGNMENT        = 'solace_meta_title_alignment';
	const DISABLE_HEADER         = 'solace_meta_disable_header';
	const DISABLE_TITLE          = 'solace_meta_disable_title';
	const DISABLE_FEATURED_IMAGE = 'solace_meta_disable_featured_image';
	const DISABLE_FOOTER         = 'solace_meta_disable_footer';
	const ELEMENTS_ORDER         = 'solace_post_elements_order';
	const SHOW_AVATAR            = 'solace_meta_author_avatar';
	/**
	...
	public function init() {
		add_filter( 'solace_sidebar_position', array( $this, 'filter_sidebar_position' ) );
		add_filter( 'solace_container_class_filter', array( $this, 'filter_container_class' ), 100 );
		add_filter( 'body_class', array( $this, 'body_classes' ) );
		add_filter( 'solace_filter_toggle_content_parts', array( $this, 'filter_components_toggle' ), 100, 2 );
		...
		add_filter( 'solace_layout_single_post_elements_order', array( $this, 'filter_post_elements' ) );
		add_filter( 'solace_title_alignment_style', array( $this, 'filter_title_alignment_style' ), 10, 2 );
		add_filter( 'solace_display_author_avatar', array( $this, 'filter_author_avatar_display' ), 15 );
		add_filter( 'solace_meta_content_width', array( $this, 'get_content_width' ) );
	}
	...
	private function get_post_id() {
		...
		$post_id = apply_filters( 'solace_post_meta_filters_post_id', $post->ID );
	...
	public function editor_content_width() {
		...
		wp_add_inline_style( 'solace-gutenberg-style', $style );
	...
	public function content_width() {
		...
		if ( $container === 'contained' ) {
			$extra_css = sprintf(
				'
			#content.solace-main .container .alignfull > [class*="__inner-container"],#content.solace-main .alignwide > [class*="__inner-container"]{
				max-width: %s;
			}
		...
		@media(min-width: 960px) {
			#content.solace-main ' . esc_attr( $container_class ) . '.alignfull > [class*="__inner-container"],#content.solace-main ' . esc_attr( $container_class ) . ' .alignwide > [class*="__inner-container"]{
				max-width: ' . $desktop_value . ';
			}
			#content.solace-main > ' . esc_attr( $container_class ) . ' > .row > .col{ max-width: ' . absint( $meta_value ) . '%' . esc_attr( $important ) . '; }
			body:not(.solace-off-canvas) #content.solace-main > ' . esc_attr( $container_class ) . ' > .row > .nv-sidebar-wrap,
			body:not(.solace-off-canvas) #content.solace-main > ' . esc_attr( $container_class ) . ' > .row > .nv-sidebar-wrap.shop-sidebar { max-width: ' . absint( $sidebar_width ) . '%' . esc_attr( $important ) . '; }
		}
		';

		wp_add_inline_style( 'solace-style', Dynamic_Css::minify_css( $style ) );
	}
	...
	private function add_button_shadow_styles( $style ) {
		$primary_values   = Mods::get( Config::MODS_BUTTON_PRIMARY_STYLE, solace_get_button_appearance_default() );
		$secondary_values = Mods::get( Config::MODS_BUTTON_SECONDARY_STYLE, solace_get_button_appearance_default( 'secondary' ) );
		if (
	 ...
	 * Codeinwp/solace-pro-addon/issues/999
	 * Codeinwp/solace/issues/2790
	 *
	 * @return bool
	 */
