...
class Pagination extends Base_View {
	...
	public function init() {
		...
		add_filter( 'solace_filter_main_script_localization', array( $this, 'filter_localization' ) );
		add_action( 'solace_do_pagination', array( $this, 'render_pagination' ) );
		add_action( 'solace_post_navigation', array( $this, 'render_post_navigation' ) );
	}
	...
	public function get_posts( \WP_REST_Request $request ) {
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				 ...
				 */
				do_action( 'solace_loop_entry_before' );
				 ...
				 * @since 2.11 in the /index.php
				 */
				do_action( 'solace_loop_entry_after' );
			}
			wp_reset_postdata();
	...
	public function render_pagination( $context ) {
		...
		if ( ! $this->has_infinite_scroll() ) {
			 ...
			 * @since 2.3.8
			 */
			do_action( 'solace_before_pagination' );
		}
	...
	public function render_post_navigation() {
		...
		$prev_link = sprintf(
			...
			esc_html__( 'previous', 'solace' ),
			'%title'
		);
		$next_link = sprintf(
			'<span class="nav-direction">%1$s</span><span>%2$s</span>',
			esc_html__( 'next', 'solace' ),
			'%title'
		);

		echo '<div class="' . esc_attr( apply_filters( 'solace_post_navigation_class', 'nv-post-navigation' ) ) . '">';
	...
	private function has_jump_to() {
		return get_theme_mod( 'solace_pagination_type', 'number' ) === 'jump-to';
	}
