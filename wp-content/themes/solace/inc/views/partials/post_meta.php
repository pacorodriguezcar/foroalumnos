...
namespace Solace\Views\Partials;
...
class Post_Meta extends Base_View {
	...
	public function init() {
		add_filter( 'solace_display_author_avatar', array( $this, 'should_display_author_avatar' ), 15 );
		add_action( 'solace_post_meta_archive', array( $this, 'render_meta_list' ), 10, 3 );
		add_action( 'solace_post_meta_single', array( $this, 'render_meta_list' ), 10, 3 );
		add_action( 'solace_do_tags', array( $this, 'render_tags_list' ) );
		add_action( 'wp_enqueue_scripts', array( $this, 'meta_custom_separator' ) );
		add_filter( 'solace_gravatar_args', [ $this, 'add_dynamic_gravatar' ] );
	}
	...
	public function should_display_author_avatar( $value ) {
		...
		if ( is_singular() ) {
			$show_avatar = get_theme_mod( 'solace_single_post_author_avatar', $show_avatar );
		}
	...
	public function add_dynamic_gravatar( $args_array ) {
		...
		$avatar_size = apply_filters( 'solace_author_avatar_size_filter', $avatar_size );
		...
		for ( $i = 0; $i < $order_length; $i++ ) {
			...
			switch ( $slug ) {
				case 'author':
					...
					$meta_content = str_replace( '{meta}', self::solace_get_author_meta( $pid, $show_before ), $format );
				...
				case 'date':
					...
					$show_updated_time = get_theme_mod( 'solace_show_last_updated_date', false );
					if ( is_singular( 'post' ) ) {
						$show_updated_time = get_theme_mod( 'solace_single_post_show_last_updated_date', $show_updated_time );
					}
				...
				case 'comments':
					...
					break;
				case 'reading':
					$allowed_context = apply_filters( 'solace_post_type_supported_list', [ 'post' ], 'block_editor' );
					if ( ! in_array( $post_type, $allowed_context ) ) {
						break;
					}
					$reading_time = apply_filters( 'solace_do_read_time', $pid );
					...
					break;
				case 'custom':
					$custom_meta = apply_filters( 'solace_do_custom_meta', '', $meta, $tag, $pid );
	 ...
	 * @return string | false
	 */
	public static function solace_get_author_meta( $post_id = null, $show_before = true ) {
		...
		$gravatar_args  = apply_filters(
			'solace_gravatar_args',
			array(
				'size' => 20,
			)
		);
		$display_avatar = apply_filters( 'solace_display_author_avatar', false );
		...
		$link = sprintf(
			...
			esc_attr( sprintf( __( 'Posts by %s', 'solace' ), $display_name ) ),
			$display_name
		);

		$link = apply_filters( 'solace_author_posts_link', $link );
	...
	public static function get_time_tags( $post_id = null ) {
		 ...
		 * @since 2.11
		 */
		$prefixes = apply_filters( 'solace_meta_date_prefix', $prefixes );
	...
	public static function get_comments( $post_id = null ) {
		...
		$comments = sprintf( _n( '%s Comment', '%s Comments', $comments_number, 'solace' ), $comments_number );
	...
	public function meta_custom_separator() {
		...
		$separator = get_theme_mod( 'solace_metadata_separator', esc_html( '/' ) );
		if ( is_singular() ) {
			$separator = get_theme_mod( 'solace_single_post_metadata_separator', $separator );
		}
		$separator = apply_filters( 'solace_metadata_separator_filter', $separator );
