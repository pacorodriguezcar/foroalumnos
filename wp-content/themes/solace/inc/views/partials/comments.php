...
class Comments extends Base_View {
	...
	public function init() {
		add_action( 'solace_do_comment_area', array( $this, 'render_comment_form' ) );
	...
	public function render_comment_form() {
		$display_form_first    = apply_filters( 'solace_show_comment_form_first', false );
		$comment_form_settings = solace_is_new_skin() ? $this->get_sumbit_form_settings() : array();
		...
		if ( have_comments() ) {
			...
			$is_boxed              = get_theme_mod( 'solace_comments_boxed_layout', false );
			if ( $is_boxed ) {
	...
	private function get_sumbit_form_settings() {
		...
		$form_title = get_theme_mod( 'solace_post_comment_form_title' );
		if ( ! empty( $form_title ) ) {
			$form_settings['title_reply'] = $form_title;
		}

		$submit_button_style           = get_theme_mod( 'solace_post_comment_form_button_style', 'primary' );
		$form_settings['class_submit'] = 'button button-' . esc_attr( $submit_button_style );

		$button_text = get_theme_mod( 'solace_post_comment_form_button_text' );
		if ( ! empty( $button_text ) ) {
			$form_settings['label_submit'] = $button_text;
		}

		$boxed_layout = get_theme_mod( 'solace_comments_form_boxed_layout', true );
		if ( $boxed_layout && solace_is_new_skin() ) {
	...
	private function maybe_render_comments_navigation() {
		...
		$aria_label = __( 'Comments Navigation', 'solace' );
		?>
		<nav class="nv-comment-navigation" role="navigation" aria-label="<?php echo esc_attr( $aria_label ); ?>">
			<div class="nav-links">
				<div class="nav-previous">
					<?php previous_comments_link( __( 'Previous', 'solace' ) ); ?>
				</div>
				<div class="nav-next">
					<?php next_comments_link( __( 'Next', 'solace' ) ); ?>
				</div>
			</div>
		</nav>
		<?php
		...
		switch ( $comment->comment_type ) {
			...
			case 'trackback':
				...
				<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
					<p>
						<?php
						echo esc_html__( 'Pingback:', 'solace' ) . '&nbsp;';
						comment_author_link();
						edit_comment_link( '(' . esc_html__( 'Edit', 'solace' ) . ')', '&nbsp;<span class="edit-link">', '</span>' );
						?>
					</p>
				</li>
				<?php
							...
							<div class="comment-author vcard">
								...
								<a href="<?php echo esc_url( get_comment_link() ); ?>">
									<time class="entry-date published"
										...
										echo sprintf( esc_html__( '%1$s at %2$s', 'solace' ), esc_html( get_comment_date() ), esc_html( get_comment_time() ) );
										?>
									</time>
								</a>
							</div>
						...
						<div class="nv-comment-content comment nv-content-wrap">
							...
							<?php if ( '0' === $comment->comment_approved ) { ?>
								<p class="comment-awaiting-moderation">
									<?php echo esc_html__( 'Comment awaiting moderation.', 'solace' ); ?>
								</p>
							<?php } ?>
						</div>
						<?php
	...
	private function render_edit_reply_link( $args, $depth ) {
		...
		<div class="edit-reply">
			<?php edit_comment_link( '(' . esc_html__( 'Edit', 'solace' ) . ')' ); ?>
			<?php
			comment_reply_link(
				array_merge(
					$args,
					array(
						'reply_text' => esc_html__( 'Reply', 'solace' ),
						'add_below'  => 'comment',
						'depth'      => $depth,
	...
	private function get_comments_title() {
		...
		$comments_title = get_theme_mod( 'solace_post_comment_section_title' );
		if ( empty( $comments_title ) ) {
			return apply_filters( 'solace_filter_comments_title', $empty_comments_title );
		}
		...
		return apply_filters( 'solace_filter_comments_title', $comments_title );
	}
