...
class Page_Header extends Base_View {
	 ...
	 */
	public function run() {
		$header_hook = get_theme_mod( 'solace_enable_featured_post', false ) && is_home() ? 'solace_do_featured_post' : 'solace_page_header';
	...
	private function get_the_page_title( $context ) {
		...
		if ( $context === 'search' ) {
			...
			$title_args['string']     = sprintf( esc_html__( 'Search Results for: %s', 'solace' ), get_search_query() );
			$title_args['wrap-class'] = 'nv-big-title';
		}
