...
class Breadcrumbs extends Base_View {
	...
	public function init() {
		add_action( 'solace_pro_hfg_breadcrumb', array( $this, 'render_breadcrumbs' ) );
	...
	private function load_theme_breadcrumbs() {
		...
		$breadcrumbs_hooks = apply_filters(
			'solace_breadcrumbs_locations',
			array(
				'solace_before_page_title',
				'solace_before_post_title',
			)
		);
	...
	/**
	 * Render breadcrumbs in Solace theme.
	 *
	 * @return bool | void
	 */
	...
	public function is_breadcrumb_enabled() {
		...
		if ( ! apply_filters( 'solace_show_breadcrumbs', true ) ) {
			return false;
		}
	...
	public static function maybe_render_seo_breadcrumbs( $html_tag, $check = false ) {
		...
		if ( function_exists( 'yoast_breadcrumb' ) ) {
			...
			if ( $yoast_breadcrumbs_enabled ) {
				yoast_breadcrumb( '<' . esc_html( $html_tag ) . ' class="nv--yoast-breadcrumb solace-breadcrumbs-wrapper">', '</' . esc_html( $html_tag ) . '>' );
				return true;
			}
		}
		...
		if ( function_exists( 'seopress_display_breadcrumbs' ) ) {
			...
			if ( $seopress_breadcrumbs_enabled ) {
				echo '<' . esc_html( $html_tag ) . ' class="solace-breadcrumbs-wrapper">';
