...
abstract class Control_Base {
	...
	public function render( $post_id ) {
		...
		wp_nonce_field( 'solace_meta_box_nonce', 'solace_meta_box_process' );
	}
	...
	final public function save( $post_id ) {
		if ( ! isset( $_POST['solace_meta_box_process'] ) ) {
			return;
		}
		if ( ! wp_verify_nonce( sanitize_key( $_POST['solace_meta_box_process'] ), 'solace_meta_box_nonce' ) ) {
			return;
		}
		...
		if ( isset( $_POST[ $this->id ] ) ) {
			...
			if ( $value === $this->settings['default'] && $this->id !== 'solace_meta_content_width' ) {
			...
			if ( $this->id === 'solace_meta_enable_content_width' ) {
				update_post_meta( $post_id, 'solace_meta_enable_content_width', 'off' );
			} else {
				delete_post_meta( $post_id, $this->id );
			}
			return;
