...
class Core_Loader {
	...
	public function update_content_import_flag() {
		update_option( 'solace_imported_demo', 'yes' );
	}
	...
	public function check_new_user() {
		$new = get_option( 'solace_new_user' );
		if ( $new === 'yes' ) {
			return true;
		}

		$install_time = get_option( 'solace_install' );
		$now          = get_option( 'solace_user_check_time' );

		if ( empty( $now ) ) {
			$now = time();
			update_option( 'solace_user_check_time', $now );
		}
		...
		if ( ( $now - $install_time ) <= 60 ) {
			update_option( 'solace_new_user', 'yes' );

			return true;
		}

		update_option( 'solace_new_user', 'no' );
	...
	private function define_modules() {
		...
		$this->features = apply_filters( 'solace_filter_main_modules', $features );
	}
	...
	private function is_fse_child_theme() {
		...
		if ( ! defined( 'SOLACE_FSE_MODE' ) ) {
			return false;
		}

		if ( SOLACE_FSE_MODE !== true ) {
			return false;
		}
