...
class Amp {
	...
	public function register_hooks() {
		...
		add_filter( 'solace_caret_wrap_filter', array( $this, 'amp_dropdowns' ), 10, 2 );
		add_filter(
			'solace_woocommerce_sidebar_filter_btn_data_attrs',
			array(
				$this,
		...
		add_filter(
			'solace_filter_sidebar_close_button_data_attrs',
			array(
				$this,
		...
		add_filter( 'solace_sidebar_data_attrs', array( $this, 'add_woo_sidebar_attrs' ), 10, 2 );
		add_filter( 'solace_search_menu_item_filter', array( $this, 'add_search_menu_item_attrs' ), 10, 2 );
		add_action( 'solace_after_header_hook', array( $this, 'render_amp_states' ) );
		add_filter( 'solace_nav_toggle_data_attrs', array( $this, 'add_nav_toggle_attrs' ) );
	...
	public function wrap_content( $item_output, $item, $depth, $args ) {
		...
		$item_output = apply_filters( 'solace_caret_wrap_filter', $item_output, $item->menu_order );
	...
	public function add_nav_toggle_attrs( $input = '' ) {
		$input  = ' on="tap:solace_body.toggleClass(class=\'is-menu-sidebar\'),AMP.setState( { nvAmpMenuExpanded: ! nvAmpMenuExpanded } )" ';
		$input .= ' role="button" ';
	...
	public function amp_dropdowns( $output, $id ) {
		...
		$state = 'solaceMenuItemExpanded' . $id;
	...
	private function maybe_add_amp_infinite_scroll() {
		...
		remove_all_actions( 'solace_do_pagination' );
		add_action( 'solace_before_footer_hook', [ $this, 'wrap_footer_before' ] );
		add_action( 'solace_after_footer_hook', [ $this, 'wrap_footer_after' ] );
	...
	public function should_display_infinite_scroll() {
		...
		$pagination_type = get_theme_mod( 'solace_pagination_type', 'number' );
	...
	public function blog_has_sidebar() {
		$option           = 'solace_default_sidebar_layout';
		$advanced_options = get_theme_mod( 'solace_advanced_layout_options', false );
		if ( $advanced_options !== false ) {
			$option = 'solace_blog_archive_sidebar_layout';
		}

		return apply_filters( 'solace_sidebar_position', get_theme_mod( $option, 'right' ) ) !== 'full-width';
	}
	...
	private function get_amp_pagination_data() {
		...
		foreach ( $pagination as $page ) {
			...
			$amp_pagination[] = [
				'url'   => $url,
				'title' => get_bloginfo( 'name' ) . ' - ' . __( 'Page', 'solace' ) . ' ' . $page . ' - ' . get_bloginfo( 'description' ),
				'image' => $image,
			];
