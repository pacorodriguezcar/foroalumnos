		...
		<div class="topic-wrap <?php wpforo_topic_types( $topic ); wpforo_unread($topic['topicid'], 'topic'); ?>">
          <div class="wpforo-topic">
			  <div class="wpforo-topic-icon">
				<div style="border: 1px dotted #999; border-radius: 5px; width: 40px; height: 38px; display: flex; justify-content: center; align-items: center;">
					<?php wpforo_topic_icon( $topic['topicid'], 'mixed' ); ?>
				</div>	
			  </div>
              <div class="wpforo-topic-info">
                <p class="wpforo-topic-title"><?php wpforo_topic_title($topic, $topic_url, '{p}{au}{t}{/a}{n}{v}') ?></p>
              ...
              <div class="wpforo-topic-stat-combined">
                  <div class="stat-line"><?php wpforo_phrase('Posts') ?>: <?php echo intval($topic['posts']) ?></div>
                  <div class="stat-line"><?php wpforo_phrase('Views') ?>: <?php echo intval($topic['views']) ?></div>
              </div>
              <div class="wpforo-topic-toggle">
                  <i id="button-arrow-<?php echo intval($topic['topicid']) ?>" class="topictoggle wpfcl-0 fas fa-chevron-<?php echo ( $post_toglle == 1 ? 'up' : 'down' ) ?>" title="<?php wpforo_phrase('First post and replies') ?>"></i>
              </div>
              <br class="wpf-clear">
          </div><!-- wpforo-topic -->
          <div class="wpforo-last-posts-<?php echo intval($topic['topicid']) ?>" style="display: <?php echo ( $post_toglle == 1 ? 'block' : 'none' ); ?>">
              <div class="wpforo-last-posts-list">
                <ul>
                    <li>
                        <div class="wpforo-last-post-title"><i class="fas fa-feather-alt wpfsx" title="<?php echo esc_attr(wpforo_phrase('Topic Author', false)) ?>"></i> &nbsp; <a href="<?php echo esc_url((string) $first_post['url']) ?>"><?php echo esc_html( wpforo_text($first_post['body'], wpforo_setting( 'topics', 'layout_extended_intro_posts_length' ), false)) ?></a></div>
